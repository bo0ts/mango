...
// This should work, I just don't want it to override the inherited method yet
...
void PyEngine::executeCallback(mpy_Object *obj, int event_type){
    ...
    callback_name = static_cast<PyEngine *>(Mango::Engine)->getObjectEventMethodName(event_type);
    ...
    if (result == NULL){
        ...
        throw PythonScriptError(obj->internalObject->objectType(), callback_name, "");
    }
    // We don't care about the result, DECREF it
    Py_DECREF(result);
}
