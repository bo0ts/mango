...
To get started writing Mango simulations as Python scripts, all you need is Mango 
and a text editor. You can download and install Mango HERE. If you're not sure which 
is an appropriate editor, consider one of THESE.

An Elephant
============

Let us try our hand at our first Mango simulation, a "hello world" of sorts. Using your
favorite text editor, save the following script as hello_elephant.py:
...
down both and drag to zoom. You should be looking at something like:

  .. image:: python/hello_box1.png
     :width: 320
     :height: 240

If you are wondering about the elephant, it is inside the box. Now this is a simple 
script, but since this is our first, let's break it down:
...
Ok, so the box is cool, but despite the fact that it actually contains an elephant, you
find yourself wanting more. So let's try another simple script that will widen our 
horizons, if only by a bit. Save the following script as hello_tree.py, and run it
like you did before:
